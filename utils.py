...
def summarize_results(
    ...
    print(true_bubble_list)
    true_bubble_list = torch.cat(true_bubble_list, dim=0).cpu()  # shape = (N, num_days)

    pstart_idx_pred_list = torch.log(
        torch.cat(start_idx_pred_list, dim=0).detach().cpu()
    )
    pend_idx_pred_list = torch.log(torch.cat(end_idx_pred_list, dim=0).detach().cpu())

    num_bubble_true_list = torch.stack(
        num_bubble_true_list, dim=0
    ).cpu()  # shape = (bs, max_bubbles)

    num_bubble_pred_list = torch.cat(num_bubble_pred_list, dim=0).detach().cpu()
    num_bubble_pred_list = torch.argmax(num_bubble_pred_list, dim=-1)

    best_span = (
        get_best_span(pstart_idx_pred_list, pend_idx_pred_list).int().cpu().numpy()
    )
    ...
    true_bubble_list = true_bubble_list.reshape_as(span_pred_list)
    em = get_EM(span_pred_list, true_bubble_list)
    em_bubble_only = get_EM_bubble_only(
        span_pred_list, true_bubble_list, num_bubble_true_list
    )
    ...
    precision_span, recall_span, f1_span, _ = get_f1(span_pred_list, true_bubble_list)
